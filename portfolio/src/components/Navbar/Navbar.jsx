...
const Navbar = ({ homeRef, aboutRef, skillsRef, servicesRef, projectsRef, certificatesRef, scrollToSection }) => {
  const [isContactModalOpen, setIsContactModalOpen] = useState(false);
  const [activeSection, setActiveSection] = useState('Home'); // Set the initial active section to Home
  ...
  }, [homeRef, aboutRef, skillsRef, servicesRef, projectsRef, certificatesRef]);
      ...
      <div className='fixed z-50 flex justify-center w-full max-w-3xl px-2 py-3 my-5 rounded-full shadow-md bg-gradient-to-r from-slate-100 via-slate-200 to-slate-300 dark:from-neutral-700 dark:via-neutral-800 dark:to-neutral-800'>
        <ul className='flex items-center gap-10 font-semibold text-[15px] text-slate-800 dark:text-neutral-300'>
          {['Home', 'About', 'Skills', 'Services', 'Projects', 'Certificates'].map((section, index) => {
            const sectionRefs = [homeRef, aboutRef, skillsRef, servicesRef, projectsRef, certificatesRef];
            return (
              <li
                key={section}
                ...
                onClick={() => {
                  ...
                  scrollToSection(sectionRefs[index]); // Call the scroll function
                }}
              >
                {section}
                <span
